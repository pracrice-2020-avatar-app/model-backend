...
#
# usage : python open_mvg_pipline.py
#
...
def detect_sfm(requestId):
    ...
    u = cv2.imread(input_eval_dir + '/0.jpg')
    print(str(1.2 * max(u.shape[:-1])))
    pIntrisics = subprocess.Popen( [os.path.join(OPENMVG_SFM_BIN, "openMVG_main_SfMInit_ImageListing"),  "-i", input_dir, "-o", matches_dir, "-d", camera_file_params, '-f', str(1.2 * max(u.shape[:-1]))])
    pIntrisics.wait()
    ...
    pMatches = subprocess.Popen( [os.path.join(OPENMVG_SFM_BIN, "openMVG_main_ComputeMatches"),  "-i", matches_dir+"/sfm_data.json", "-o", matches_dir, "-f", "1", "-n", "AUTO", '-g', 'f'] )
    pMatches.wait()
